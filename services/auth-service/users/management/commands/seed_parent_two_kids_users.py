from datetime import timedelta

from django.conf import settings
from django.core.management.base import BaseCommand, CommandError
from django.db import transaction
from django.utils import timezone

from users.models import CustomUser, GuardianInvitation, Kid
from users.serializers import CustomTokenObtainPairSerializer
from users.services import accept_guardian_invitation
from users.tokens import KidRefreshToken

DEV_PASSWORD = 'DevPass123!'

DEFAULT_PARENT = 'dev_parent_multi'
DEFAULT_KID1 = 'dev_kid_one'
DEFAULT_KID2 = 'dev_kid_two'


def _slug(value: str) -> str:
    cleaned = ''.join(
        ch if ch.isalnum() or ch in ('_', '-') else '_'
        for ch in value.strip().lower().replace(' ', '_')
    )
    cleaned = cleaned.strip('_')
    if not cleaned:
        raise CommandError('Name/username cannot be empty.')
    return cleaned


class Command(BaseCommand):
    help = (
        'Create one parent with two kids (parent is primary guardian of both) '
        'and print JWT tokens for Swagger/curl.'
    )

    def add_arguments(self, parser):
        parser.add_argument(
            '--parent', type=str, default='',
            help=f'Parent username (default: {DEFAULT_PARENT})',
        )
        parser.add_argument(
            '--kid1', type=str, default='',
            help=f'First kid username (default: {DEFAULT_KID1})',
        )
        parser.add_argument(
            '--kid2', type=str, default='',
            help=f'Second kid username (default: {DEFAULT_KID2})',
        )
        parser.add_argument('--name1', type=str, default='', help='First kid display name')
        parser.add_argument('--name2', type=str, default='', help='Second kid display name')
        parser.add_argument(
            '--force',
            action='store_true',
            help='Run even when DEBUG is False (local dev only).',
        )

    def handle(self, *args, **options):
        if not settings.DEBUG and not options['force']:
            raise CommandError(
                'Refusing to seed when DEBUG is False. Use --force if you mean it.'
            )

        parent_username = _slug(options['parent'] or DEFAULT_PARENT)
        kid1_username = _slug(options['kid1'] or DEFAULT_KID1)
        kid2_username = _slug(options['kid2'] or DEFAULT_KID2)
        if kid1_username == kid2_username:
            raise CommandError('Kid usernames must be different.')

        parent_email = f'{parent_username.replace("_", "-")}@localhost'
        kids = (
            {
                'username': kid1_username,
                'email': f'{kid1_username}@localhost',
                'name': (options['name1'] or '').strip()
                or kid1_username.replace('_', ' ').title(),
            },
            {
                'username': kid2_username,
                'email': f'{kid2_username}@localhost',
                'name': (options['name2'] or '').strip()
                or kid2_username.replace('_', ' ').title(),
            },
        )

        with transaction.atomic():
            parent = self._ensure_parent(parent_email, parent_username)
            seeded = [self._ensure_kid(data, parent, parent_email) for data in kids]

        parent_access = str(
            CustomTokenObtainPairSerializer.get_token(parent).access_token
        )

        self.stdout.write(
            self.style.SUCCESS(
                '\nParent with two kids ready (auth-service) — 1 parent, 2 kids:\n'
            )
        )
        self.stdout.write(f'  Password (all):   {DEV_PASSWORD}')
        self.stdout.write(f'  Parent  email:    {parent_email}')
        self.stdout.write(f'  Parent  username: {parent_username}')
        self.stdout.write(f'  Parent user ID:   {parent.id}\n')

        exports = [f'  export PARENT_ACCESS="{parent_access}"']
        self.stdout.write(f'  PARENT_ACCESS={parent_access}\n')
        for index, (data, kid) in enumerate(zip(kids, seeded), start=1):
            kid_access = str(KidRefreshToken.for_kid(kid).access_token)
            self.stdout.write(f'--- Kid {index} ---')
            self.stdout.write(f'  Kid     username: {data["username"]}')
            self.stdout.write(f'  Kid     name:     {data["name"]}')
            self.stdout.write(f'  Kid     email:    {data["email"]}')
            self.stdout.write(f'  Kid ID:           {kid.id}')
            self.stdout.write(f'  KID{index}_ACCESS={kid_access}')
            self.stdout.write(f'  KID{index}_ID={kid.id}\n')
            exports.append(f'  export KID{index}_ACCESS="{kid_access}"')
            exports.append(f'  export KID{index}_ID="{kid.id}"')

        self.stdout.write('Or export in your shell:\n')
        for line in exports:
            self.stdout.write(line)
        self.stdout.write('')

    def _ensure_parent(self, email: str, username: str) -> CustomUser:
        parent = CustomUser.objects.filter(email=email).first()
        if parent is None:
            return CustomUser.objects.create_user(
                email=email,
                username=username,
                password=DEV_PASSWORD,
                role='parent',
                email_verified=True,
            )

        parent.email_verified = True
        parent.role = 'parent'
        parent.username = username
        update_fields = ['email_verified', 'role', 'username']
        if not parent.check_password(DEV_PASSWORD):
            parent.set_password(DEV_PASSWORD)
            update_fields.append('password')
        parent.save(update_fields=update_fields)
        return parent

    def _ensure_kid(self, data: dict, parent: CustomUser, parent_email: str) -> Kid:
        kid = Kid.objects.filter(username=data['username']).first()
        if kid is None:
            kid = Kid(
                name=data['name'],
                username=data['username'],
                email=data['email'],
                email_verified=True,
                registration_status=Kid.RegistrationStatus.AWAITING_PRIMARY_PARENT,
            )
            kid.set_password(DEV_PASSWORD)
            kid.save()
        else:
            kid.email_verified = True
            kid.email = data['email']
            kid.name = data['name']
            update_fields = ['email_verified', 'email', 'name']
            if not kid.check_password(DEV_PASSWORD):
                kid.set_password(DEV_PASSWORD)
                update_fields.append('password_hash')
            kid.save(update_fields=update_fields)

        invitation = kid.guardian_invitations.filter(
            invite_email__iexact=parent_email,
            role='primary',
        ).first()
        if invitation is None:
            invitation = GuardianInvitation.objects.create(
                kid=kid,
                invite_email=parent_email,
                role='primary',
                status='pending',
                created_by_kid=True,
                expires_at=timezone.now() + timedelta(days=7),
            )
        if invitation.status != 'accepted':
            accept_guardian_invitation(invitation, parent)
        kid.refresh_from_db()
        return kid
